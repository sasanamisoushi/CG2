...
# 1. Fix UpdateCamera
# We will match from `void Player::UpdateCamera(Camera *camera, const Vector3 *targetPos) {` to the end of the function (before `Vector3 Player::GetForwardVector() const {`)
camera_pattern = re.compile(r"(void Player::UpdateCamera\(Camera \*camera, const Vector3 \*targetPos\) \{).*?(^\s*\}\s*Vector3 Player::GetForwardVector\(\) const \{)", re.MULTILINE | re.DOTALL)
...
text = camera_pattern.sub(fixed_camera, text)
print("Updated UpdateCamera!")

# 2. Fix Move
move_pattern = re.compile(r"(	// ==================================\s*if \(LengthSq\(localMove\) > 0\.0001f\) \{).*?(		if \(LengthSq\(velocity_\) <= 0\.00001f\) \{)", re.MULTILINE | re.DOTALL)
...
fixed_move = r"""	// ==================================
	...
	if (LengthSq(localMove) > 0.0001f) {
		...
		if (currentMode_ == PlayerMode::Fighter) {
			auto input = Input::GetInstance();
			bool moveForward = input->PushKey(DIK_W);
			bool moveBackward = input->PushKey(DIK_S);
...
text = move_pattern.sub(fixed_move, text)
print("Updated Move!")
