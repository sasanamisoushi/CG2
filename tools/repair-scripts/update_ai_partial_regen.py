...
# 1. Replace MYADDON_OT_ai_edit_selected_enemy_path
old_edit_op = """class MYADDON_OT_ai_edit_selected_enemy_path(bpy.types.Operator):
...
new_edit_op = """class MYADDON_OT_randomize_ai_seed(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for path_obj in selected_paths:
            ...
            if provider == 'GEMINI':
                ...
                points = _build_ai_enemy_points(style, motion, spawn, player, center, extents, side, 0, rng, Vector((0,0,0)))
                loop = motion["loop"] if motion.get("loop") is not None else style == 'PATROL'
                speed = 0.050 * motion.get("speed_multiplier", 1.0)
                handle_type = 'VECTOR' if motion.get("pattern") == "EDGE_ORBIT" and motion.get("respect_bounds") else 'AUTO'
...
text = text.replace(old_edit_op, new_edit_op)

# 2. Add MYADDON_OT_randomize_ai_seed to classes
old_classes = """    MYADDON_OT_ai_generate_enemy_plan,
    MYADDON_OT_ai_edit_selected_enemy_path,
    MYADDON_OT_create_enemy_path,"""

new_classes = """    MYADDON_OT_randomize_ai_seed,
