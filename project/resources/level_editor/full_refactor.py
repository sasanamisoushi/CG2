...
# Allow Exception bubble up in request_gemini_enemy_plan
pattern_exc = re.compile(r"except Exception as exc:\s*print\(f\"Gemini Enemy Plan error: \{exc\}\"\)\s*return None", re.MULTILINE)
text = pattern_exc.sub("except Exception as exc:\n        print(f\"Gemini Enemy Plan error: {exc}\")\n        raise exc", text)

text = text.replace("_sanitize_gemini_enemy_plan", "_sanitize_enemy_plan_data")

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(text)
print("Refactoring and patching complete.")
